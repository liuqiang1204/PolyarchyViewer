...
/**
 * This is my extension of the JLabel Required because we need to store
 * additional information with the label Rather than store the information in a
 * another data structure use the labels We need them for operations so it make
 * sense to keep the information together
 * 
...
public class AMLabel extends JLabel {
	...
	/**
	 * not available = -1, collapsed = 0, tmp expanded = 1, expanded = 2 the
	 * image changed when 1 and 2 --qiang 5 - highlighted
	 */
	...
	/**
	 * Status control: Behaviors: enter leave click doubleclick check status:
	...
	/**
	 * Default constructor Everything is set to empty or 0
	 */
	public AMLabel() {
		super("");
		add_Contents("", "", "0", "", 1, 0);
	...
	/**
	 * This constructor sets all of the strings to the parameter The secondary
	 * string is set to a substring of the parameter @param text
	 * 
	 * @param text
	 *            - The string to which all of the strings will be set
	 */
	...
	/**
	 * This constructor specifies the text values No text operations are
	 * performed
	 * 
	 * @param text
	 *            - The text to be displayed
	 * @param primary
	 *            - The primary text for the label
	 * @param secondary
	 *            - The secondary text for the label
	 */
	...
	/**
	 * This constructor sets all of the values No text operations are performed
	 * 
	 * @param text
	 *            - The text to be displayed
	 * @param primary
	 ...
	 * 
	 * @param text
	 *            -The text to be used for the primary, display and secondary
	 *            text
	 * @param id
	 *            - The unique id to be used
	 */
	 ...
	 * 
	 * @param text
	 *            - The text for the primary text and to be shortened for the
	 *            display text
	 * @param id
	 *            - The unique id of the label
	 * @param image
	 *            - The icon to be added to the label
	 */
	 ...
	 * 
	 * @param image
	 *            - The image to be used in the label
	 */
	 ...
	 *            - The image to be used in the label
	 * @param positioning
	 *            - the positioning of the icon in the label
	 */
	...
	/**
	 * Add the details to the label This is more generic to have this in a
	 * method rather than in the constructors
	 * 
	 * @param primary
	 ...
	 *            - its new position
	 */
	public void add_Contents(String primary, String secondary, String id,
			String web, int its_level, int its_position) {
	...
	/**
	 * Used to paint the rectangle on top of the label Only set of labels that
	 * are defined as being a bar Set the count (total) and sub_count
	 * (comparison) before entering this method
	 */
	protected void paintComponent(Graphics g) {
		...
		// if this is an image, then we need to constantly constrain the
		// size
...
//			this.setLocation(new Point(x,y+5));
			...
			// if the bar width is greater than the text then the text will
			// fit
			if (getBar_width() > adv) {
				adv = getBar_width();
			}
			...
			g2.setColor(GlobalConstants.bar_foreColor);

			// set the bar to a preferred size of the label that will allow
			// the text to fit
			...
			// If we have a count
			if (count > 0) {
				// The percentage is equal to the sub count / the count or
				// total
				// Needs to be a double for precision or no result will be
				// calculated
				if (sub_count >= count) {
					percentage = 1;
				} else {
			...
			// get the width we want to fill as a percentage of the total
			// width
			...
			if (!count_option.equals("numerical")) {
				...
				g2.fillRect(0, 0, fill_width, bar_height-1);
			}
...
//			}
	...
	/**
	 * This method creates the secondary text Fundamentally it splits the string
	 * on spaces for abbreviated labels
	 * 
	 * @param text
	 *            - The text to be split and abbreviated
	...
	public static String text_split_letter(String text) {
		...
		if (text.length() >= endIndex) {
			...
			// now loop through the words and take the first letter to be used
			// to shorten the word
	...
	/**
	 * Set the parent of this label Required for knowing something about the
	 * hierarchical structure
	 * 
	 * @param new_parent
	 *            - The unique id of the parent for this label
	 */
	...
	/**
	 * Set the bar for this label Required for instant access to the bar The
	 * has_bar is set by this method
	 * 
	 * @param new_bar
	 *            - The bar for this label
	 */
	...
	/**
	 * Set the image for this label Required so we can perform operations
	 * quickly on it The has_image is set by this method
	 * 
	 * @param image
	 *            - The image label
	 */
	...
	/**
	 * Changes the state of the click If it was clicked then it is removed If it
	 * wasn't clicked then it is
	 */
	public void clicked() {
	 ...
	 * 
	 * @param visible
	 *            - what the visibility will be set to
	 */
	 ...
	 * 
	 * @param entry
	 *            - If the event was an entry or an exit
	 */
	...
	/**
	 * Set the label so that when we repaint it it will have its original
	 * appearance
	 */
	...
	/**
	 * We want to clear this label Some logic here depending on the level of the
	 * label and its contents
	 * 
	 * @param label
	 *            - The label to be cleared
	 * @param remove_count
	 *            - if we want to remove the sub count
	 */
	...
	/**
	 * Clear this label and its corresponding labels If it has a bar that will
	 * be cleared as will the image
	 * 
	 * @param remove_count
	 *            - if we want to remove the count
	 */
