 ...
 * All of the objects that can be seen by the user are stored here
 * Or at least are extanisated here
...
public class View extends JFrame {
	 ...
	 */
	private String[] look_options_available = { "NimbusLookAndFeel", "MetalLookAndFeel", "WindowsLookAndFeel", "WindowsClassicLookAndFeel", "MotifLookAndFeel" };
	...
	/**
	 * the object to be used to connect to the database, used here if we want to update data on the database
	 */
	 ...
	 * Sets up the whole display
	 * @param model - Takes the model containing the data.  For this example it is the database connector
	 */
	 ...
	 * @param model - the java database model
	 */
	public void addContents(Java_Connector model) {
        ...
        global_options.setLayout(new BoxLayout(global_options, BoxLayout.Y_AXIS));
        ...
        try{
        if(main_table.next())
        	hierarchy1 = new Hierarchy(main_table.getInt(1), main_table.getString(2));
        	
        if(main_table.next())
			hierarchy2 = new Hierarchy(main_table.getInt(1), main_table.getString(2)); 
		      
        if(main_table.next())
			hierarchy3 = new Hierarchy(main_table.getInt(1), main_table.getString(2));       
        }
       
        catch(SQLException se){
        	se.printStackTrace();
        	Custom_Messages.display_error(this, "Adding Hierarchy", "ERROR: An error has occurred adding the Hierarchy to the display");
        }
	 ...
	 * @param combo_box - the combo box listener from the controller
	 * @param scroll_listener - the listener for the scroll bar
	 * @param clear_listener - the listener for the clear button
	 * @param keypress - the keypress for the search box
	 */
	public void add_listeners(ComboBox_Action combo_box, Scroll_adjust_listener scroll_listener, Clear_Button_Action clear_listener, KeyPress_Action keypress) {
		...
		hierarchy1.add_listeners(c_l, scroll_listener, clear_listener, keypress);
		hierarchy2.add_listeners(c_l, scroll_listener, clear_listener, keypress);
		hierarchy3.add_listeners(c_l, scroll_listener, clear_listener, keypress);
	 ...
	 * @param hierarchy - defines which hierarchy the label's will be added to
	 * @param level - Its level with the hierarchy, 1, 2 or 3
	 * @param name - The text to be used for the label
	 * @param id - The unique id of the label
	 * @param its_parent - the parent id for this label
	 * @return - the Label that was created
	 */
	public AMLabel addLabel(Hierarchy hierarchy, int level, String name, String id, String its_parent) {
		
		//remove any of the white space in the string
		name = name.trim();
		
		//remove all non ASCII characters, can make the display ugly and cause problem
	...
	/**
	 * This actually adds the label
	 * First we need to check that the label will be placed in the correct position
	 * Level 1 labels at the top
	 * Level 2 labels on the second row
	 * Level 3 labels from row 3 - max rows
	 * @param this_level - Which level in the hierarchy it will be added to
	 * @param label - The label to be added
	 * @param this_hierarchy - The hierarchy that the label will be added to
	 */
	public void addLabel(int this_level, AMLabel label, Hierarchy this_hierarchy) {
		...
		constraint.insets = new Insets(0, padding, 0, panel.getWidth() - (padding + 15));
		...
		//make sure that we don't already has this id, if we do problems will occur
		if(result != null) {
	 ...
	 * @param hierarchy_num - the hierarchy number, starting from 0
	 * @return - the hierarchy in that position
	 */
	public Hierarchy decide_hierarchy(int hierarchy_num) {
	
		//find which hierarchy want to add the contents and get the relevant information
	    ...
	    default:
	    	...
	    	Custom_Messages.display_error(this, "ERROR", "ERROR: An invalid hierarchy level has been specified");
	    	return null;
	    }
	}
	
	/**
	 * When a duplicate key has been detected we should alert somebody that this has happened
	 * @param key - The key for the hastable
	 * @param new_value - The new value to be placed at that key
	 * @param old_value - The previous value which is being replaced
	 */
	public void duplicate_key(String key, Integer new_value, Integer old_value) {
		Custom_Messages.display_error(this, "Duplicate Key", "ERROR: Duplicate key: " + key + "\n" +
				" new value: " + new_value + " old value: " + old_value );
	}
	 ...
	 * @param override - if we want to override the load and show the status
	 */
	public void startGlass(boolean override) {
	    ...
	    //if it vis is loaded and we don't override it then don't show the progress bar
	    if(loaded && !override) {
	 ...
	 * @param override - if we want to override glass and remove it
	 */
	...
	/**
	 * Increment the value of the waiter for the progress bar
	 * Without a value we increment the waiter by 1
	 * @param value - the new value of the waiter
	 */
	...
	/**
	 * This sets the look and feel of the view
	 * This is a global setting and will change the look of every GUI element inside the window
	 * @param look - the look we want as an integer
	 */
	public void lookANDfeel(int look) {
		...
		switch (look) {
		    ...
		    default:
		    	//something else has been specified, give an error message
		    	Custom_Messages.display_error(this, "ERROR", "ERROR: An invalid look has been selected\n" +
		    			"The valid range is from 1 to " + (look_options_available.length + 1) + "\n" +
		...
		//Catch the error that occurs and display some information
			...
			Custom_Messages.display_error(this, "ERROR", "An error has occured when changing the look and feel of the display");
			e.printStackTrace();
		}
	}
	...
	/**
	 * Setup the bar with new information
	 * This method is required as we now have information to add into the bar that we previously didn't have
	 * Also used to change the count of the bar when we decide to count a different category
	 * @param count - The count for this bar
	 * @param hierarchy - that the bar belongs to
	 * @param label - The label that the bar belongs to
	 * @param left_padding - The padding to be added to the left of the bar for consistent spacing
	 */
	public void bar_edit(double count, Hierarchy hierarchy, AMLabel label) {
		...
		if(label.isHas_bar()) {
			...
			//get the width of the upper panel because we want that to determine the size
...
//			count = Math.round(count*100)/100.0d;
	...
	/**
	 * Decide the external padding that is required for this object to be properly spaced
	 * The bar size should be relative to its size and to the largest
	 * The largest bar should reach all the way to the other side of the panel
	 * @param count - The count for the label
	 * @param largest - The largest to be compared with
	 * @param panel_width - The width of the panel above the bar
	 * @param left_padding - The padding to be added
	 * @param bar - The bar itself
	 * @return - the inset that has the padding added to it
	 */
	public Insets decideSpace(double count, float largest, float panel_width, int left_padding, AMLabel bar) {
		
		//now we try and find the width of this bar
		//we want the largest bar to fill the screen and the others to be relative
		if(bar.isIs_bar()) {
			
			//subtract the left padding on both sides so that we have a buffer
			panel_width -= (left_padding * 1.5);
			
			//find the width of this bar compared to the largest, this is a percentage
			double this_width_per = count / largest;

			//we don't want the label to exceed the size of the panel with the left padding being added
	...
	/**
	 * Clear all of the panels with information
	 * Also clear the corresponding information box
	 * We can only clear the panels if they are not locked
	 */
	 ...
	 * @param source_button - the button that was pressed
	 */
	...
	/**
	 * We are moving a panel one position
	 * We will decide in which direction it will move and to where
	 * @param label - the label where the interaction started
	 */
	public void move_left_right(JLabel label) {
		...
		//Check which hierarchy the label has came from, need to get the last character from the label's name as an integer
		int index = Character.getNumericValue(label.getName().charAt(label.getName().length() -1));
	 ...
	 * @param total - the total number of panels
	...
	/**
	 * Perform some action on the global panel
	 * This can move, show, hide the panel
	 * @param label - the option label that was interacted with
	 */
	 ...
	 * @param set - if we want the cursor to be waiting or normal
	 */
	...
	/**
	 * Hide or show a hierarchy depending on the label where the interaction started
	 * If we hide the panel we want the other elements to be smaller to save space
	 * When showing the panel it should be set back
	 * @param label - the label that was interacted with
	 * @param visible - if we want it to be visible or not
	 */
	...
	/**
	 * Find a specific hierarchy depending on the string id we are given
	 * Check this against the hasmap's
	 ...
	 * @param option - the option we want to change to
	 */
	...
	/*LISTENERS*/
	 ...
	 /**
     * This listens to the labels that are on top of the panel and perform actions on the panel
     * @author Anthony Scata
     * @version 1.3
     *
     */
    public class Control_Listener extends MouseAdapter {
		 ...
		 * Perform the event now as that is what usually happens with these types of events
		 */
		public void mouseReleased(MouseEvent e) {
			...
			//get the labels new position on the screen as it may have moved vertically during the interaction
			//this causes problems with the cursor then hovering over a new label and creating new interaction
			Point new_label_position = label.getLocationOnScreen();

			//if the label has moved vertically past the size of the label
			if(new_label_position.y != old_label_position.y || new_label_position.x != old_label_position.x) {
				robot.mouseMove(new_label_position.x + width, new_label_position.y + 2);
			}
		}
    }
	...
	/**
	 * @param countOptions the count_options to set
	 */
	...
	/**
	 * @param optionsAvailable the optionsAvailable to set
	 */
	...
	/**
	 * @param barOptions the bar_options to set
	 */
	...
	/**
	 * @param barOptionsAvailable the bar_options_available to set
	 */
